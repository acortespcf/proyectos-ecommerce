---
name: Feature Request
about: Solicitud de nueva funcionalidad o mejora
title: "[FEAT] Descripción breve"
labels: 'feature'
assignees: ''

---

## 📋 Descripción
<!-- Descripción clara del feature solicitado -->

## 🎯 Contexto
<!-- Por qué es importante? Impacto en usuarios, negocio o performance -->

## ✅ Criterios de Aceptación
<!-- Qué debe cumplirse para considerar esto como "done" -->
- [ ] 
- [ ] 
- [ ] 

## 🛠️ Tareas Técnicas
<!-- Steps concretos de implementación si es necesario -->
- [ ] 
- [ ] 

## 🌿 Rama Propuesta
```
feat/chile-[ISSUE-NUMBER]-<descripcion-en-kebab-case>
```
**Ejemplo:** `feat/chile-1234-schema-markup-vuelta-clases`

## 📌 Scope
<!-- Selecciona el área afectada -->
- [ ] seo
- [ ] gmb
- [ ] modyo
- [ ] analytics
- [ ] integraciones
- [ ] otro

## ⚡ Consideraciones
<!-- Performance, SEO, seguridad, compatibilidad, etc -->

## 🔗 Links Relacionados
<!-- PRs, documentación, tickets en otros sistemas, wireframes -->